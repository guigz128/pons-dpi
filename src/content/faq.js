export const faqItems = [
  {
    question: 'Quels diagnostics sont obligatoires pour vendre mon bien ?',
    answer:
      "Cela dépend de l'année de construction, du type de bien et de sa localisation. En général : DPE, amiante (avant 1997), plomb (avant 1949), électricité et gaz (installations de plus de 15 ans), termites (zone à risque), ERP, et mesurage Carrez (copropriété). Utilisez notre outil de devis en ligne pour obtenir la liste exacte en quelques clics.",
  },
  {
    question: 'Combien coûte un diagnostic immobilier à Montpellier ?',
    answer:
      'Le prix dépend du type de bien, de sa surface et du nombre de diagnostics nécessaires. Nous proposons des tarifs transparents et des packs avantageux dès 3 diagnostics. Demandez votre devis en ligne pour un prix exact et sans engagement.',
  },
  {
    question: 'Quel est le délai pour obtenir un rendez-vous ?',
    answer:
      "Nous intervenons généralement sous 48 heures. Pour les urgences, contactez-nous directement par téléphone ou WhatsApp pour trouver un créneau au plus vite.",
  },
  {
    question: 'Combien de temps dure une intervention ?',
    answer:
      "La durée dépend du type de diagnostic et de la taille du bien. Comptez environ 1 heure pour un DPE d'appartement, et 2 à 3 heures pour un pack complet en maison. Nous nous adaptons à vos disponibilités.",
  },
  {
    question: 'Les rapports sont-ils transmis aux notaires ?',
    answer:
      "Oui, tous les rapports sont transmis au format numérique et peuvent être envoyés directement à votre notaire, agent immobilier ou syndic. Ils sont conformes aux exigences réglementaires et lisibles par tous.",
  },
  {
    question: "Que se passe-t-il si mon DPE est classé F ou G ?",
    answer:
      "Un logement classé F ou G est considéré comme une passoire thermique. Depuis 2023, un audit énergétique est obligatoire pour la vente de maisons individuelles classées F ou G — il doit être réalisé par un professionnel disposant de la qualification correspondante (OPQIBI, RGE Études). Je vous orienterai vers un partenaire de confiance si votre DPE conduit à cette obligation.",
  },
  {
    question: 'Intervenez-vous en dehors de Montpellier ?',
    answer:
      "Oui, nous intervenons sur l'ensemble de la métropole de Montpellier et ses alentours : Castelnau-le-Lez, Lattes, Pérols, Mauguio, Saint-Jean-de-Védas, Juvignac, Grabels, Jacou, Le Crès, Clapiers, Prades-le-Lez, Saint-Gély-du-Fesc, et plus encore. Contactez-nous pour vérifier votre commune.",
  },
]
